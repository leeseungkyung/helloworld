...
//				frame.setVisible(false); // 끄는건 전체적인 GUI 자체가 Frame으로 감싸져 있기 때문에 frame을 키고 끄고 해야함.
		...
		Korea.setResizable(false); // 크기변경불가
		
		
		
		
		
		//한식 분류체계만 Label로 뿌려주기								by 태영
		// A : 한식, B : 패스트푸드	C : 치킨,	 D : 피자
		String setCat = "A";
		RNameL = new ArrayList<String>();
		RNameL = rdao.getSelectRestaurantName(setCat);
		

		JLabel restaurant[] = new JLabel[RNameL.size()];			// 한식분류된 사이즈만큼
		JPanel panelCol[] = new JPanel[RNameL.size()];			// 한식분류된 사이즈만큼
		int increate= 10;											// label 세로 초기 위치
		for (int i = 0; i < RNameL.size(); i++) {
			restaurant[i] = new JLabel(RNameL.get(i));
			panelCol[i] = new JPanel();
			panelCol[i].setSize(200, 400);							//panelCol[i]에 대한 조절
			...
			restaurant[i].setFont(new Font("나눔고딕", Font.PLAIN, 20));
